...
var tokenHiveSlice4UsageLogColumns = []string{
	...
	"group_id", "subscription_id", "input_tokens", "output_tokens", "cache_creation_tokens", "cache_read_tokens",
	...
	"video_resolution", "video_duration_seconds", "service_tier", "reasoning_effort", "inbound_endpoint",
	"upstream_endpoint", "cache_ttl_overridden", "long_context_billing_applied", "channel_id", "model_mapping_chain",
	"billing_tier", "billing_mode", "account_stats_cost", "session_id", "created_at",
}
...
func TestTokenHiveSlice4UsageInsertWhitelistConflictAndNullability(t *testing.T) {
	...
	require.Len(t, args, 58, "batch input prepends one synthetic input_idx to the 57 whitelisted values")
	...
	require.Equal(t, int16(service.RequestTypeSync), prepared.args[28], "legacy zero values normalize to sync at the pinned request_type index")
	...
	require.Len(t, bestEffortArgs, 57)
}
...
func TestTokenHiveSlice4RetentionPartitionAndRowBoundaries(t *testing.T) {
	...
	t.Run("partitioned drops only months before the cutoff month", func(t *testing.T) {
		...
		mock.ExpectQuery("SELECT c.relname").WillReturnRows(sqlmock.NewRows([]string{"relname"}).
			...
			AddRow("other_202501"))
		mock.ExpectExec(regexp.QuoteMeta(`DROP TABLE IF EXISTS "usage_logs_202512"`)).WillReturnResult(sqlmock.NewResult(0, 0))
		mock.ExpectExec(regexp.QuoteMeta(`DROP TABLE IF EXISTS "usage_logs_202601"`)).WillReturnResult(sqlmock.NewResult(0, 0))
	...
	t.Run("unpartitioned removes rows strictly before the UTC cutoff", func(t *testing.T) {
		...
		mock.ExpectExec("WHERE created_at < \\$1").
