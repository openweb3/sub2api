...
func TestOpenAIGatewayServiceForwardImages_APIKeyZeroImageResponseIsNotSuccessful(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.Nil(t, result)
	require.Zero(t, rec.Body.Len(), "zero-image upstream body must not be committed as a successful client response")
}
