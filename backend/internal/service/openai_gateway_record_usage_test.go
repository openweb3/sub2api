...
func TestOpenAIGatewayServiceRecordUsage_PrefersClientRequestIDOverUpstreamRequestID(t *testing.T) {
	...
	require.True(t, ok)
	metadata, metadataErr := BuildTokenHiveMetadata(ctx, 10049, http.MethodPost, "https://api.openai.com/v1/responses", "gpt-5.1", SourceOperationOpenAIResponsesHTTP, tokenHiveAccount, []byte("slice-1-test-tenant-hmac-key-32bytes"))
