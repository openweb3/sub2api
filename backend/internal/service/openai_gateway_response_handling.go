...
func (s *OpenAIGatewayService) handleStreamingResponseWithReasoningAndPolicy(ctx context.Context, resp *http.Response, c *gin.Context, account *Account, startTime time.Time, originalModel, mappedModel, reasoningEffort string, policy TokenHiveResponsePolicy) (*openaiStreamingResult, error) {
	...
	finalizeStream := func() (*openaiStreamingResult, error) {
		...
		if policy.AllowAccountMutation && codexFailureTerminal && sawBareError && !sawResponseFailed && bareErrorAccountSideEffectsPending {
	...
	processSSELine := func(line string, queueDrained bool) {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if eventType == "response.failed" || eventType == "error" {
				...
				if policy.AllowAccountMutation && outputStarted && !cyberHit {
					if codexFailureTerminal && eventType == "error" {
				...
				if !outputStarted {
					...
					if policy.AllowAccountFailover && shouldFailover {
						sawFailedEvent = true
					...
					if !cyberHit && !sawBareError && !policy.Dedicated {
