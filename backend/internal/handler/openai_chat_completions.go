...
func (h *OpenAIGatewayHandler) ChatCompletions(c *gin.Context) {
	...
	for {
			...
			if result != nil && result.ImageCount > 0 {
				...
				if failoverErr, allowFailover := gatewayFailoverForPolicy(err, responsePolicy); allowFailover {
					...
					if responsePolicy.AllowSchedulerFeedback && failoverErr.ShouldReportAccountScheduleFailure() {
						h.gatewayService.ReportOpenAIAccountScheduleResultWithPolicy(responsePolicy, account.ID, account.GetMappedModel(reqModel), false, nil)
					}
		...
		if result != nil {
			h.gatewayService.ReportOpenAIAccountScheduleResultWithPolicy(responsePolicy, account.ID, account.GetMappedModel(reqModel), true, result.FirstTokenMs)
		} else {
			h.gatewayService.ReportOpenAIAccountScheduleResultWithPolicy(responsePolicy, account.ID, account.GetMappedModel(reqModel), true, nil)
		}
