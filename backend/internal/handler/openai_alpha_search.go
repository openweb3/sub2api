...
func (h *OpenAIGatewayHandler) AlphaSearch(c *gin.Context) {
	...
	for {
		...
		if err == nil {
			h.gatewayService.ReportOpenAIAccountScheduleResultWithPolicy(responsePolicy, account, openAIAccountScheduleModel(c, account, requestedModel, false, result), true, nil)
			if result != nil {
		...
		if !errors.As(err, &failoverErr) {
			h.gatewayService.ReportOpenAIAccountScheduleResultWithPolicy(responsePolicy, account, openAIAccountScheduleModel(c, account, requestedModel, false, result), false, nil, err)
