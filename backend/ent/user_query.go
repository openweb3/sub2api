...
func (_q *UserQuery) QueryBees() *BeeQuery {
...
func (_q *UserQuery) WithBees(opts ...func(*BeeQuery)) *UserQuery {
	query := (&BeeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withBees; query != nil {
		if err := _q.loadBees(ctx, query, nodes,
			func(n *User) { n.Edges.Bees = []*Bee{} },
			func(n *User, e *Bee) { n.Edges.Bees = append(n.Edges.Bees, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadBees(ctx context.Context, query *BeeQuery, nodes []*User, init func(*User), assign func(*User, *Bee)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(bee.FieldUserID)
	}
	query.Where(predicate.Bee(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.BeesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
